...
from notifications.utils import create_notification, create_proposal_status_notification
...
class EditProposalViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        """
        if self.action in ['list', 'retrieve', 'moderate']:
            return [IsAdminUser()]
    ...
    def moderate(self, request, pk=None):
        ...
        if action == 'approve':
            ...
            create_proposal_status_notification(proposal.user, proposal, 'approved')
            ...
            # Отправляем уведомление пользователю
            create_proposal_status_notification(proposal.user, proposal, 'rejected')
